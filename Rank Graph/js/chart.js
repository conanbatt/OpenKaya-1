...
function draw(target, data) {
    ...
    svg = d3.select(target).select('svg').select('g');
    if (svg.empty()) {
    ...
    circles
      ...
      $('svg circle').tipsy({ 
        ...
        title: function() {
          var d = this.__data__;
      var pDate = d.date;
          return d.gamelink + '<br />Date: ' + pDate + '<br />Rank: ' + d.value + '<br />Result:' + d.result + '<br />Opponent: ' + d.opponent;
        }
      });
}
    ...
    var getData = $.ajax({
        ...
        success: function(data) {
            if(data.length > 0) {
                ...
                draw(target, resultsArray.reverse());
                fixRanks();
            } else {
...
function graphSettings(e) {
    target      = "#user-chart";
    gamecount   = $("input[name='gamecount']").val() || 20;
    username    = $("input[name='username']").val();

    // Check username exists;
    if(username == 'fakeman') { // Exists query?
        $('#usernamewarn').fadeIn();
        setTimeout(function() {
    ...
    console.log(target + " : " + username + " : " + gamecount);
    getGraph(target, username, gamecount);
    e.preventDefault();
};
